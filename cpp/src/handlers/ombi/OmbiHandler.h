...
class OmbiHandler : public BaseHandler {
public:
    explicit OmbiHandler(std::shared_ptr<DiscordMessageService> discord)
    ...
    void handle(const std::string& eventName, const nlohmann::json&, const EventData&) override {
        spdlog::warn("OmbiHandler not yet implemented (event: {})", eventName);
    }
